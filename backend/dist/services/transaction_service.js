...
exports.getGlobalTransactions = exports.processTransfer = exports.processWithdrawal = exports.processStaffDeposit = void 0;
...
//     doc.end();
// }
const processStaffDeposit = async (accountId, amount, staffId, referenceNote) => {
    if (amount <= 0)
        throw new AppError_1.AppError(400, "Deposit amount must be greater than zero.");
    ...
    if (account.status !== 'ACTIVE')
        throw new AppError_1.AppError(403, `Cannot deposit. Account is ${account.status}.`);
...
exports.processStaffDeposit = processStaffDeposit;
const processWithdrawal = async (accountId, amount, staffId, referenceNote) => {
    if (amount <= 0)
        throw new AppError_1.AppError(400, 'Amount must be greater than zero');
    const result = await db_1.prisma.$transaction(async (tx) => {
        const account = await tx.account.findUnique({ where: { id: accountId } });
        if (!account)
            throw new AppError_1.AppError(404, 'Account not found');
        if (account.status !== 'ACTIVE')
            throw new AppError_1.AppError(400, `Cannot withdraw from ${account.status} account`);
        if (Number(account.balance) < amount)
            throw new AppError_1.AppError(400, 'Insufficient funds');
...
const processTransfer = async (fromAccountId, toAccountNumber, amount, staffId, referenceNote) => {
    if (amount <= 0)
        throw new AppError_1.AppError(400, 'Amount must be greater than zero');
    const result = await db_1.prisma.$transaction(async (tx) => {
        // Check Sender
        ...
        if (!sender)
            throw new AppError_1.AppError(404, 'Sender account not found');
        if (sender.status !== 'ACTIVE')
            throw new AppError_1.AppError(400, 'Sender account is not active');
        if (Number(sender.balance) < amount)
            throw new AppError_1.AppError(400, 'Insufficient funds for transfer');
        // Check Receiver
        const receiver = await tx.account.findUnique({ where: { accountNumber: toAccountNumber } });
        if (!receiver)
            throw new AppError_1.AppError(404, 'Destination account not found');
        if (receiver.status !== 'ACTIVE')
            throw new AppError_1.AppError(400, 'Destination account is not active');
        if (sender.id === receiver.id)
            throw new AppError_1.AppError(400, 'Cannot transfer to the same account');
...
exports.getGlobalTransactions = getGlobalTransactions;
