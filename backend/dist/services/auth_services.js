...
exports.getLoanAccountDetails = exports.updateCustomerStatus = exports.getCustomerProfile = exports.searchCustomers = exports.createAccount = exports.getStaffProfile = exports.loginStaff = void 0;
const db_1 = require("../utils/db");
const AppError_1 = require("../error/AppError");
const bcryptjs_1 = __importDefault(require("bcryptjs"));
const account_number_generation_1 = require("./account_number_generation");
...
const loginStaff = async (data) => {
    ...
    if (!identifier || !password) {
        throw new AppError_1.AppError(400, 'Username/Email and password are required');
    }
    //  Query Staff by username or email
    const staff = await db_1.prisma.staff.findFirst({
        where: {
            OR: [
                { email: identifier },
                { username: identifier }
    ...
    if (!staff || !staff.isActive) {
        throw new AppError_1.AppError(401, 'Invalid credentials or inactive account');
    }
    //  Verify password hash
    const isMatch = await bcryptjs_1.default.compare(password, staff.passwordHash);
    if (!isMatch) {
...
const getStaffProfile = async (staffId) => {
    if (!staffId) {
        throw new AppError_1.AppError(400, 'Staff ID is required');
    }
    const staff = await db_1.prisma.staff.findUnique({
        where: { id: staffId },
        select: {
            id: true,
            username: true,
...
const createAccount = async (staffId, accountCategory, firstName, lastName, email, phoneNumber, address, initialDeposit, loanAmount, loanTerm) => {
    const isLoan = accountCategory === "LOAN";
    if (isLoan) {
        if (!loanAmount || Number(loanAmount) <= 0) {
            throw new AppError_1.AppError(400, "Loan Principal Amount must be greater than zero.");
        }
        if (!loanTerm || Number(loanTerm) <= 0) {
            throw new AppError_1.AppError(400, "A valid loan term duration in months is required.");
        }
    }
    ...
    const result = await db_1.prisma.$transaction(async (tx) => {
        ...
        const accountNumber = await (0, account_number_generation_1.generateUniqueAccountNumber)(tx);
...
const searchCustomers = async (searchQuery, page = 1, limit = 10) => {
    let whereClause = {};
    if (searchQuery.trim()) {
        whereClause = {
            OR: [
    ...
    const [customers, totalCount] = await db_1.prisma.$transaction([
        ...
        db_1.prisma.customer.count({ where: whereClause }) // Get total matching rows
    ]);
...
const getCustomerProfile = async (customerId, page, limit) => {
    const skip = (page - 1) * limit;
    ...
    const [transactions, totalCount] = await db_1.prisma.$transaction([
...
const updateCustomerStatus = async (accountId, status, staffId) => {
    const allowedStatuses = ['ACTIVE', 'FROZEN', 'CLOSED'];
    if (!status || !allowedStatuses.includes(status.toUpperCase())) {
        throw new AppError_1.AppError(404, 'Invalid Status');
    }
    const result = await db_1.prisma.$transaction(async (tx) => {
...
const getLoanAccountDetails = async (accountId) => {
    ...
    if (!account || account.accountType !== 'LOAN') {
        throw new AppError_1.AppError(404, 'Loan account not found or invalid account category.');
    }
    // 2. Find the initial disbursement transaction to extract the loan term
    const disbursementTx = await db_1.prisma.transaction.findFirst({
        where: {
...
exports.getLoanAccountDetails = getLoanAccountDetails;
